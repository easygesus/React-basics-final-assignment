import { Stack, Text, Container, Input, Center, Image, Heading, Button as CButton, SimpleGrid } from "@chakra-ui/react";
...
/*
<Card w={{ md: "300px" }} h={{ md: "570px" }} align={"center"} mb={3} bgColor={"whiteAlpha.500"} key={obj.recipe.label}>
      <CardHeader>
        <Text align={"center"}>{obj.recipe.mealType}</Text>
        <Heading align={"center"} pt={1} size={"lg"}>
          {obj.recipe.label}
        </Heading>
      </CardHeader>
      <Stack spacing={0}>
        <CardBody maxWidth={200}>
          <Image align={"center"} borderRadius="lg" h={200} w={200} src={obj.recipe.image} alt={obj.recipe.label} />
          <Stack>
            <Text align={"center"}>
              {obj.recipe.healthLabels.some(label => label.toLowerCase() === "vegan") || obj.recipe.healthLabels.some(label => label.toLowerCase() === "vegan") ? (
                <p>
            ...
            <Text align={"center"}>
              <strong>{obj.recipe.dietLabels}</strong>
            </Text>
            <Text align={"center"}>
              Dish: <strong>{obj.recipe.dishType}</strong>
            </Text>
